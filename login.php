...
$loginrequired .= $_GET['loginrequired'];
$regdone .= $_GET['regdone'];
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    if (empty($error)) {
        if($query = $db->prepare("SELECT id, name, password FROM users WHERE email = ?")) {
            ...
            $query->execute();
			// error_log($query);
			$query->bind_result($uid, $uname, $psh);
            $row = $query->fetch();
            if ($row) {
                if (password_verify($password, $psh)) {
                    $_SESSION["userid"] = $uid;
                    $_SESSION["user"] = $row;
                    $_SESSION["name"] = $uname;
      ...
      <div class="col-md-8 col-lg-6 col-xl-4 offset-xl-1">
        <form action="" method="post">
          ...
          <div class="divider d-flex align-items-center my-4">
			...
			<?php
			if (empty($success) && empty($error) && !empty($regdone)) {
				echo "<div class=\"divider d-flex my-4\"><p class=\"text-success fw-bold mb-0\">";
				echo $regdone;
				echo "</p></div>";
			}
			elseif (empty($success) && empty($error) && !empty($loginrequired)) {
